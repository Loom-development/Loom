...
That's it. No `package.json`, no `Dockerfile`, no `docker-compose.yml`. Loom generates a working project with sensible defaults. Run it interactively and Loom will walk you through template, runtime, and database choices.
...
Loom pulls the container image once, installs your dependencies, starts your database (if you added one), waits for everything to be healthy, generates an HTTPS certificate, and maps a local domain — all automatically.
...
Your app is running. No `localhost:3000`. No port numbers to remember. No self-signed certificate warnings. Just a URL that works.
...
loom restart --recreate # clean rebuild (wipes node_modules, re-installs)
```
...
```bash
loom init rails7 --dir my-blog --db postgres
```
...
Loom generates credentials, creates the database, wires the connection URL into your `.env`, and waits for the database to accept connections before starting your app. No manual configuration.
...
```bash
cd ~/projects/api && loom start    # API server at https://api.loom.local
cd ~/projects/web && loom start    # Frontend at https://web.loom.local
```

Both run simultaneously. Each project gets its own isolated network. A shared proxy routes requests to the right container by hostname. Stop one without affecting the other.
...
```bash
loom backup db ./backups/$(date +%Y-%m-%d).sql
loom restore db ./backups/2026-01-15.sql
```

Works with MySQL, PostgreSQL, MongoDB, Redis, and SQLite. One command, no flags to memorize.
...
# Windows (PowerShell)
iwr -useb https://raw.githubusercontent.com/Loom-development/Loom/main/scripts/install.ps1 | iex
