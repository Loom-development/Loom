...
async function assertNoSymlinkPath(root, relativePath, label) {
    ...
    if (!fromRealRoot || fromRealRoot.startsWith("..") || isAbsolute(fromRealRoot))
    ...
    for (const segment of relativePath.split(/[\\/]/)) {
        ...
        try {
            if ((await lstat(current)).isSymbolicLink())
                throw new Error(`Refusing symlinked ${label} '${relativePath}'`);
        }
        catch (error) {
            if (error.code === "ENOENT")
                break;
...
export function renderProjectName(loomYaml, projectName) {
...
export function renderDatabaseService(loomYaml, db) {
    const { serviceName, serviceYaml } = buildDatabaseServiceBlock(db);
    if (new RegExp(`^ {2}${serviceName}:`, "m").test(loomYaml))
        return loomYaml;
...
async function renderCandidates(options, candidateRoot) {
    ...
    if (options.stack.loomOwnedFiles.includes("loom.yaml")) {
        ...
        for (const database of options.manifest.renderInputs.databases) {
            if (!["postgres", "mysql", "mariadb", "mongodb", "redis"].includes(database))
                throw new Error(`Unknown stored database type '${database}'`);
            content = renderDatabaseService(content, database);
        }
        await writeFile(path, content, "utf8");
    }
}
export async function planProjectUpgrade(options) {
    ...
    try {
        for (const path of Object.keys(options.manifest.ownedFiles))
            assertSafeRelativePath(path, "owned file path");
        for (const path of options.stack.loomOwnedFiles)
            assertSafeRelativePath(path, "Loom-owned asset path");
        await renderCandidates(options, candidateRoot);
        const files = [];
        for (const path of Object.keys(options.manifest.ownedFiles).sort()) {
            if (!options.stack.loomOwnedFiles.includes(path))
                throw new Error(`Stack '${options.stack.id}' does not declare owned asset '${path}'`);
...
export async function applyProjectUpgrade(plan, options) {
    ...
    try {
        for (const file of plan.files.filter((entry) => updated.includes(entry.path))) {
            ...
            if (await sha256File(staged) !== file.candidateSha256)
                throw new Error(`Candidate hash changed for '${file.path}'`);
