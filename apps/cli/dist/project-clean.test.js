...
async function fixture(generatedPaths = [
...
test("planner rejects symlinked targets and parents", async () => {
    for (const kind of ["target", "parent"]) {
        ...
        try {
            ...
            await mkdir(outside);
            if (kind === "target")
                await symlink(outside, join(value.projectRoot, "cache"));
            if (kind === "parent")
                await symlink(outside, join(value.projectRoot, "cache"));
...
test("planner discovers and protects project and workspace manifests outside generated roots", async () => {
    ...
    try {
        ...
        await assert.rejects(planProjectClean({ ...value, stack: { ...value.stack, generatedPaths: [{ path: "api", category: "dependency" }] } }), /protected/i);
    }
    finally {
        await rm(value.root, { recursive: true, force: true });
    }
...
test("executor removes only planned paths, reports missing paths, and preserves protected state", async () => {
    ...
    try {
        ...
        const snapshots = new Map(await Promise.all(preserved.map(async (path) => [path, await readFile(join(value.projectRoot, path))])));
        ...
        for (const [path, contents] of snapshots)
            assert.deepEqual(await readFile(join(value.projectRoot, path)), contents);
    }
    finally {
        await rm(value.root, { recursive: true, force: true });
    }
...
test("executor removes the complete declared Python maintenance set", async () => {
    for (const id of ["python-django", "django-react"]) {
...
//# sourceMappingURL=project-clean.test.js.map