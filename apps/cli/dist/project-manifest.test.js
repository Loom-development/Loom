...
import { buildProjectManifest, classifyProjectManifestStack, loadProjectManifest, writeProjectManifest } from "./project-manifest.js";
...
test("writeProjectManifest atomically replaces deterministic JSON", async () => {
    ...
    try {
        ...
        assert.equal(await readFile(join(targetDir, loaded.manifest.ownedFiles["loom.yaml"].baselinePath), "utf8"), await readFile(join(targetDir, "loom.yaml"), "utf8"));
    }
    finally {
        await rm(targetDir, { recursive: true, force: true });
    }
...
test("a later baseline failure cannot alter content referenced by the previous manifest", async () => {
    ...
    try {
        ...
        await assert.rejects(writeProjectManifest(targetDir, "0.3.5", nodeStack, nodeStack.loomOwnedFiles, renderInputs));
...
test("v2 definition metadata distinguishes current, explicit legacy, and incompatible manifests", async () => {
    ...
    try {
        ...
        assert.equal(loaded.kind, "ready");
        if (loaded.kind !== "ready")
            throw new Error("expected legacy-ready manifest");
        ...
        assert.equal(loaded.kind, "ready");
        if (loaded.kind !== "ready")
            throw new Error("expected parsed incompatible manifest");
        ...
        if (compatibility.kind === "incompatible")
            assert.match(compatibility.reason, /not a declared legacy scaffold version/i);
    }
    finally {
        await rm(targetDir, { recursive: true, force: true });
    }
...
//# sourceMappingURL=project-manifest.test.js.map