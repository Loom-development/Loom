...
const ready = { kind: "ready", manifest: manifestValue };
const config = {
...
const podman = { available: true, rootless: true, version: "5", machine: { supported: false, running: false } };
...
test("diagnoses manifest lifecycle and scaffold drift", async (t) => {
    const root = await fixture();
    t.after(() => rm(root, { recursive: true, force: true }));
    const run = (manifest, selected = stack) => runProjectDoctor({ projectRoot: root, config, manifest, stack: selected, probes: probes() });
...
test("reports missing and conflicting lockfiles", async (t) => {
    const root = await mkdtemp(join(tmpdir(), "loom-doctor-locks-"));
    t.after(() => rm(root, { recursive: true, force: true }));
    await writeFile(join(root, "package.json"), "{}\n");
...
test("evaluates Node lockfiles per workspace directory", async (t) => {
    const root = await mkdtemp(join(tmpdir(), "loom-doctor-workspace-locks-"));
    t.after(() => rm(root, { recursive: true, force: true }));
    ...
    assert.equal(lockfiles.status, "failure");
    assert.match(lockfiles.detail, /web\/package\.json/);
});
test("reports dependency ownership and writability", async (t) => {
    ...
    assert.equal(dependency.status, "failure");
    assert.match(dependency.detail, /not writable/);
    assert.match(dependency.detail, /owned by uid/);
});
test("parses host, IP, and TCP port mappings and reports unavailable ports", async (t) => {
    const root = await fixture();
    t.after(() => rm(root, { recursive: true, force: true }));
    const mapped = { ...config, services: { app: { ...config.services.app, ports: ["3000:3000", "127.0.0.1:4000:4000/tcp", "5000"] } }, routes: [{ host: "demo.local", service: "app", port: 4000 }] };
    const checked = [];
    ...
    assert.deepEqual(checked, [3000, 4000]);
...
test("rejects duplicate host-port claims across services before probing availability", async (t) => {
    const root = await fixture();
    t.after(() => rm(root, { recursive: true, force: true }));
    const duplicate = { ...config, services: {
    ...
    assert.equal(ports.status, "failure");
...
test("rejects malformed mappings and distinguishes route service and port failures", async (t) => {
    const root = await fixture();
    t.after(() => rm(root, { recursive: true, force: true }));
    const invalid = { ...config, services: { app: { ...config.services.app, ports: ["127.0.0.1:bad:3000"] } }, routes: [{ host: "a.local", service: "missing", port: 80 }, { host: "b.local", service: "app", port: 9999 }] };
    const results = await runProjectDoctor({ projectRoot: root, config: invalid, manifest: ready, stack, probes: probes() });
    assert.equal(results.find(({ id }) => id === "ports").status, "failure");
    const route = results.find(({ id }) => id === "routes");
    assert.equal(route.status, "failure");
    assert.match(route.detail, /does not exist/);
    assert.match(route.detail, /does not expose/);
});
test("hosts integration is checked only when routes exist and is warning-only", async (t) => {
    ...
    assert.equal(warning.find(({ id }) => id === "hosts").status, "warning");
    ...
    assert.equal(pass.find(({ id }) => id === "hosts").status, "pass");
    assert.equal(calls, 1);
});
//# sourceMappingURL=project-doctor.test.js.map