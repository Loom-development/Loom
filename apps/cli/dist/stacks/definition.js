...
export function validateGeneratorVersion(version) {
...
export function validateRuntimeImage(image) {
    if (!/^[A-Z][A-Z0-9_]*$/.test(image.env))
        throw new Error(`Runtime image environment '${image.env}' must be uppercase`);
...
export function validateStackDefinition(definition) {
    if (!stackIds.includes(definition.id))
        throw new Error(`Unknown stack id: ${definition.id}`);
    ...
    if (definition.assetPath !== `${definition.id}/templates` || !safeRelative(definition.assetPath))
        throw new Error(`Unsafe asset path: ${definition.assetPath}`);
    ...
    for (const [kind, paths] of [["generated path", generatedPaths], ["protected path", protectedPaths], ["Loom-owned path", definition.loomOwnedFiles], ["host-write path", definition.hostWrites]]) {
        for (const path of paths)
            if (!safeRelative(path) || path === ".loom" || path.startsWith(".loom/"))
    ...
    if (!Number.isInteger(definition.readiness.timeoutSeconds) || definition.readiness.timeoutSeconds <= 0 || !definition.readiness.value.trim())
        throw new Error("Readiness requires a value and positive timeout");
    assertSortedUnique(definition.compatibility.architectures, "architectures");
}
export function defineStack(definition) {
