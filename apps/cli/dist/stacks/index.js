...
export * from "./pins.js";
const compatibility = { architectures: ["arm", "arm64", "x64"], runtime: "podman-rootless" };
const generated = (path, category) => ({ path, category });
...
const migratedDefinitions = new Map([
...
export const stackDefinitions = stackIds.map((id) => migratedDefinitions.get(id) ?? compatibilityDefinition(id));
const stackDefinitionsById = new Map(stackDefinitions.map((definition) => [definition.id, definition]));
export function findStackDefinition(stackId) { return stackDefinitionsById.get(stackId); }
export function listStackIds() { return stackDefinitions.map(({ id }) => id).sort(); }
