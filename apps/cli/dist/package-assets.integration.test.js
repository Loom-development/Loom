...
async function npmPackageFiles() {
...
function assertCanonicalStackAssets(paths, stacksPrefix) {
    ...
    assert.equal(stackPaths.filter((path) => /^[^/]+\/stack\.js$/.test(path.slice(stacksPrefix.length))).length, 31, "expected one compiled definition for every public stack ID");
...
function hasLegacyExampleAssets(paths, parent = "") {
...
test("npm installs the produced CLI tgz and executes its binary without workspace dependencies", async () => {
    ...
    try {
        ...
        const installedPackage = JSON.parse(await readFile(resolve(installRoot, "node_modules", "@loomdev", "cli", "package.json"), "utf8"));
        ...
        const manifest = JSON.parse(await readFile(resolve(targetDir, ".loom", "manifest.json"), "utf8"));
...
async function isolatedCliFixture(projectDirName) {
...
test("isolated packaged CLI initializes Node from canonical stack assets", async () => {
    ...
    try {
        ...
        const manifest = JSON.parse(await readFile(resolve(value.targetDir, ".loom", "manifest.json"), "utf8"));
...
test("release archives publish canonical stack assets without legacy examples", async () => {
    ...
    for (const archive of ["loom-windows-x64.zip", "loom-windows-arm64.zip"]) {
        ...
        const paths = JSON.parse(stdout).sort();
...
//# sourceMappingURL=package-assets.integration.test.js.map