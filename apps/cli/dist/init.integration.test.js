...
test("init php with --db adds database service", async () => {
    ...
    const manifest = JSON.parse(await readFile(join(targetDir, ".loom", "manifest.json"), "utf8"));
...
test("init applies runtime image overrides from --image", async () => {
    ...
    assert.equal(await readFile(join(targetDir, manifest.ownedFiles["loom.yaml"].baselinePath), "utf8"), generatedConfig);
});
...
test("init with --blank-template deletes existing files and copies the full template", async () => {
    ...
    await assert.rejects(() => readFile(join(targetDir, "existing.txt"), "utf8"), /ENOENT/);
});
