...
export const initTemplateDescriptions = {
...
export const initImageChoicesByTemplate = {
...
export async function chooseInitTemplate(suggestedTemplate, input = process.stdin, output = process.stdout) {
    ...
    try {
        ...
        throw new Error(`Unknown template selection '${answer}'. Choose a number from 1 to ${initTemplateChoices.length} or a valid template name.`);
    }
    finally {
        rl.close();
    }
}
export function describeInitTemplate(template) {
    return initTemplateDescriptions[template] ?? "Loom project template.";
}
export async function chooseInitImageOverrides(template, currentValues, lockedEnvKeys = [], input = process.stdin, output = process.stdout) {
    ...
    try {
        ...
        for (const choice of imageChoices) {
            ...
            throw new Error(`Unknown ${choice.label} selection '${answer}'. Choose a number from 1 to ${choice.options.length} or a listed image tag.`);
        }
        return selected;
    }
    finally {
...
export const supportedDbTypes = ["postgres", "mysql", "mariadb", "mongodb", "redis"];
const dbTypeDescriptions = {
    postgres: "PostgreSQL",
    mysql: "MySQL",
    mariadb: "MariaDB",
    mongodb: "MongoDB",
...
export function isValidDbType(value) {
    return supportedDbTypes.includes(value);
}
function describeDbType(db) {
    return dbTypeDescriptions[db];
}
export async function chooseInitDatabases(input = process.stdin, output = process.stdout) {
    if (!input.isTTY) {
        return [];
    }
    const rl = createInterface({ input, output });
