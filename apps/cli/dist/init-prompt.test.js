...
test("chooseInitTemplate accepts numeric selection", async () => {
    ...
    assert.match(captured, /12\. php-drupal - Drupal served by Apache \+ PHP\./);
...
test("chooseInitImageOverrides skips locked env keys", async () => {
    ...
    const selected = await chooseInitImageOverrides("php", { PHP_IMAGE: "docker.io/library/php:8.4-apache" }, ["PHP_IMAGE"], input, output);
...
test("chooseInitImageOverrides rejects invalid runtime selections", async () => {
    ...
    const output = new PassThrough();
    input.end("unknown\n");
    await assert.rejects(() => chooseInitImageOverrides("node", { NODE_IMAGE: "docker.io/library/node:24-alpine" }, [], input, output), /Unknown Node runtime selection/);
});
//# sourceMappingURL=init-prompt.test.js.map