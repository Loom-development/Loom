...
import { prepareInitTarget as prepareInitTargetFromDefinition, runDrupalCreateProjectWithDependencies, runRailsCreateProjectWithDependencies, runRailsHotwireCreateProjectWithDependencies, runStackGeneratorWithDependencies, runSymfonyCreateProjectWithDependencies, runWordPressCreateProjectWithDependencies } from "./init-template.js";
import { findStackDefinition } from "./stacks.js";
function prepareInitTarget(template, targetDir, blankTemplate, dependencies = {}) {
...
test("prepareInitTarget returns config-only result for non-empty generic targets without --blank-template", async () => {
    assert.deepEqual(await prepareInitTarget("php", "/workspace/app", false, {
...
test("prepareInitTarget rejects non-empty Drupal targets even when forced", async () => {
    await assert.rejects(() => prepareInitTarget("php-drupal", "/workspace/drupal", true, {
        directoryHasFiles: async () => true
...
test("prepareInitTarget rejects non-empty WordPress targets even when forced", async () => {
    await assert.rejects(() => prepareInitTarget("php-wordpress", "/workspace/wordpress", true, {
        directoryHasFiles: async () => true
...
test("prepareInitTarget rejects non-empty Rails 7 targets even when forced", async () => {
    await assert.rejects(() => prepareInitTarget("rails7", "/workspace/rails7", true, {
        directoryHasFiles: async () => true
...
test("prepareInitTarget rejects non-empty Rails 7 + Hotwire targets even when forced", async () => {
    await assert.rejects(() => prepareInitTarget("rails7-hotwire", "/workspace/rails-hotwire", true, {
        directoryHasFiles: async () => true
...
test("prepareInitTarget rejects non-empty Symfony targets even when forced", async () => {
    await assert.rejects(() => prepareInitTarget("php-symfony", "/workspace/symfony", true, {
        directoryHasFiles: async () => true
...
test("runDrupalCreateProjectWithDependencies reports when Podman is unavailable", async () => {
    await assert.rejects(() => runDrupalCreateProjectWithDependencies("/workspace/drupal", {
        runCommand: async () => {
            const error = new Error("podman missing");
            error.code = "ENOENT";
            throw error;
        }
    }), /Install Podman/i);
...
test("runWordPressCreateProjectWithDependencies reports when Podman is unavailable", async () => {
    await assert.rejects(() => runWordPressCreateProjectWithDependencies("/workspace/wordpress", {
        runCommand: async () => {
            const error = new Error("podman missing");
            error.code = "ENOENT";
            throw error;
...
test("runRailsCreateProjectWithDependencies reports when Podman is unavailable", async () => {
    await assert.rejects(() => runRailsCreateProjectWithDependencies("/workspace/rails7", {
        runCommand: async () => {
            const error = new Error("podman missing");
            error.code = "ENOENT";
            throw error;
...
test("runRailsHotwireCreateProjectWithDependencies reports when Podman is unavailable", async () => {
    await assert.rejects(() => runRailsHotwireCreateProjectWithDependencies("/workspace/rails-hotwire", {
        runCommand: async () => {
            const error = new Error("podman missing");
            error.code = "ENOENT";
            throw error;
...
test("runSymfonyCreateProjectWithDependencies reports when Podman is unavailable", async () => {
    await assert.rejects(() => runSymfonyCreateProjectWithDependencies("/workspace/symfony", {
        runCommand: async () => {
            const error = new Error("podman missing");
            error.code = "ENOENT";
            throw error;
...
test("classified bootstrap errors identify every public stack ID", async () => {
    ...
    for (const testCase of cases) {
        ...
        await assert.rejects(() => runStackGeneratorWithDependencies(definition, `/workspace/${testCase.id}`, {
...
test("all bootstrap definitions resolve exact packages and gem installs", async () => {
    for (const id of ["php-drupal", "php-symfony", "php-wordpress", "rails7", "rails7-hotwire"]) {
...
//# sourceMappingURL=init-template.test.js.map