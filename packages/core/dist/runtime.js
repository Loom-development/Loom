...
function assertLinuxRootlessRuntimeReady(config, dependencies) {
    ...
    throw new Error(`Rootless Podman requires a writable user runtime directory, but '${runtimeDir}' does not exist. ` +
        `Log in with a real user session or enable lingering with 'loginctl enable-linger ${uid}', then retry 'loom start'.`);
}
export async function ensureRuntimeReady(config, dependencies) {
