...
import { requireRestoreSupport, resolveRestoreInputPath, restoreConfiguredService } from "./restore.js";
...
test("requireRestoreSupport rejects unsupported service types", () => {
    assert.throws(() => requireRestoreSupport("cache", "redis", ["postgres", "mysql"]), /not currently supported by 'loom restore'/i);
});
...
test("requireRestoreSupport explains the current SQL Server limitation", () => {
    assert.throws(() => requireRestoreSupport("db", "sqlserver", defaultOrchestratorDependencies.supportedRestoreServiceTypes), /SQL Server restore is not yet supported/i);
});
