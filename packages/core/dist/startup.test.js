...
import { formatStartupNotice, formatHttpsInfo, formatProxyPorts, formatRouteBindings, formatStartedService, formatStartHeader } from "./startup.js";
test("formatStartHeader and formatStartedService create startup progress lines", () => {
    assert.equal(formatStartupNotice(), "Note: Startup may take a few minutes while Loom downloads images and installs dependencies. Starts using cached resources are usually faster.\n");
...
test("formatRouteBindings renders protocol-specific route lines", () => {
    assert.deepEqual(formatRouteBindings([
        {
            host: "demo.test",
            service: "app",
            targetPort: 3000,
...
test("formatProxyPorts and formatHttpsInfo render summary lines", () => {
    assert.equal(formatProxyPorts(8080, 8443), "Route proxy listener ports: http://localhost:8080 https://localhost:8443 (use with configured route hostnames)\n");
