...
function createConfig(serviceType) {
    return {
        version: 1,
        name: "demo",
...
test("startConfiguredService starts php services, ensures composer, and waits for readiness", async () => {
    const events = [];
    const output = [];
    await startConfiguredService(createConfig("php"), "app", "demo-net", {
...
test("startConfiguredService writes periodic waiting updates before reporting the service as started", async () => {
    const output = [];
    await startConfiguredService(createConfig("node"), "app", "demo-net", {
...
//# sourceMappingURL=service-start.test.js.map