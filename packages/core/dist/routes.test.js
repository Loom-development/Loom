...
test("publishConfiguredRoutes skips proxy setup and output when no routes exist", async () => {
    ...
    const lines = [];
    await publishConfiguredRoutes(createConfig(), [], "demo-net", {
...
test("publishConfiguredRoutes configures proxy and writes route and https summaries", async () => {
    ...
    const proxyCalls = [];
    const lines = [];
    await publishConfiguredRoutes(createConfig(), [
        {
            host: "demo.test",
            service: "app",
            targetPort: 3000,
...
test("publishConfiguredRoutes prints /etc/hosts instructions when hosts could not be written", async () => {
    const lines = [];
    const errors = [];
    await publishConfiguredRoutes(createConfig(), [{ host: "demo.test", service: "app", targetPort: 3000, externalPort: 8080, https: true }], "demo-net", {
