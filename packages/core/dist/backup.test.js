...
import { backupAllConfiguredServices, backupConfiguredService, listBackupSupportedServices, requireBackupExtension, resolveBackupOutputPath } from "./backup.js";
test("listBackupSupportedServices filters services by supported backup type", () => {
    ...
    const supported = listBackupSupportedServices(config, (serviceType) => ["postgres", "redis"].includes(serviceType) ? "dump" : null);
    assert.deepEqual(supported.map(([serviceName]) => serviceName), ["db", "cache"]);
});
test("requireBackupExtension returns the extension for supported services", () => {
    const extension = requireBackupExtension("db", "postgres", ["postgres", "redis"], (serviceType) => (serviceType === "postgres" ? "sql" : null));
    assert.equal(extension, "sql");
});
test("requireBackupExtension throws for unsupported services", () => {
    assert.throws(() => requireBackupExtension("app", "node", ["postgres", "redis"], () => null), /not currently supported/i);
});
test("resolveBackupOutputPath uses explicit and generated output paths", () => {
    ...
    const generated = resolveBackupOutputPath("/workspace", "demo", "db", "sql", undefined, new Date("2026-03-11T10:20:30.000Z"));
...
test("backupAllConfiguredServices rejects when no supported backup services exist", async () => {
    ...
    await assert.rejects(() => backupAllConfiguredServices(config, "/workspace", {
