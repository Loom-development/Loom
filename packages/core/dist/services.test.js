...
import { buildServiceNotFoundError, getConfiguredService, requireConfiguredService } from "./services.js";
const config = {
    version: 1,
    name: "demo",
...
test("requireConfiguredService rejects with a service-not-found error", async () => {
    await assert.rejects(() => requireConfiguredService(config, "worker", async () => []), /Service 'worker' is not defined in loom.yaml/i);
});
