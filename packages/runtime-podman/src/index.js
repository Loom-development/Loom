...
export function containerName(projectName, serviceName) {
    return `${projectName}-${serviceName}`;
}
export async function ensurePodmanNetwork(networkName) {
...
export async function listProjectContainers(projectName) {
...
export async function waitForServiceReady(projectName, serviceName, options) {
...
export async function ensureServiceStarted(projectName, serviceName, service, networkName) {
    ...
    const args = await buildPodmanRunArgs(serviceName, name, service, networkName, expectedImage, expectedServiceHash);
...
export async function execServiceCommand(projectName, serviceName, command) {
    const name = containerName(projectName, serviceName);
    if (command.length === 0) {
...
export async function backupServiceToFile(projectName, serviceName, service, outputPath) {
    ...
    await new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            reject(new Error(`Backup failed for service '${serviceName}': ${stderr.trim() || "unknown error"}`));
        });
    });
}
export async function detectPodmanCapabilities() {
...
export async function ensureMachineRunning(managed) {
