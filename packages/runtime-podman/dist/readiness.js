...
export async function isPortOpen(port, timeoutMs) {
...
export async function arePortsReachable(ports, timeoutMs) {
    if (ports.length === 0) {
        return false;
    }
...
export async function waitForServiceReadyWithDependencies(projectName, serviceName, options, dependencies = {}) {
    ...
    const { startPeriodSeconds = 0, intervalSeconds = 2, progressIntervalSeconds = 15, retries = 30, timeoutSeconds = 2, ports = [], command } = options ?? {};
    ...
    while (attempts < maxAttempts && now() - startedAt <= timeoutMs) {
        ...
        if (!info.running) {
            throw new Error(`Container '${name}' exited before becoming ready. Check 'loom logs ${serviceName} --no-follow' for the startup failure.`);
        }
