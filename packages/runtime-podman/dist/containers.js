...
export function parseHostPorts(portMappings = []) {
    return portMappings
        .map((mapping) => {
...
export async function ensureBindMountParentDirs(volumes = []) {
...
export function serviceConfigHash(service) {
    const signature = {
        type: service.type,
...
export async function buildPodmanRunArgs(serviceName, containerNameValue, service, networkName, expectedImage, expectedServiceHash) {
    const args = [
        "run",
        "-d",
        "--name",
