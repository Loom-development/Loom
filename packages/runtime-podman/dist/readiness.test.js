...
test("waitForServiceReady accepts a healthy container", async () => {
    let calls = 0;
    await waitForServiceReadyWithDependencies("demo", "app", { retries: 3, intervalSeconds: 1 }, {
...
test("waitForServiceReady uses port reachability when ports are configured", async () => {
    let probeCalls = 0;
    await waitForServiceReadyWithDependencies("demo", "app", { ports: ["8080:80"], retries: 2, intervalSeconds: 1 }, {
        inspectContainerByName: async () => ({
            name: "demo-app",
            state: "running",
            running: true,
...
test("waitForServiceReady rejects unhealthy containers", async () => {
    await assert.rejects(() => waitForServiceReadyWithDependencies("demo", "app", { retries: 1, intervalSeconds: 1 }, {
        inspectContainerByName: async () => ({
            name: "demo-app",
            state: "running",
            running: true,
...
test("waitForServiceReady does not sleep through the full startup grace period when the container becomes healthy early", async () => {
    let calls = 0;
    await waitForServiceReadyWithDependencies("demo", "app", { retries: 3, intervalSeconds: 1, startPeriodSeconds: 300 }, {
        inspectContainerByName: async () => {
            calls += 1;
            return {
                name: "demo-app",
...
test("waitForServiceReady ignores unhealthy healthchecks during startup grace and reports progress", async () => {
    ...
    let currentTime = 0;
    const progress = [];
    await waitForServiceReadyWithDependencies("demo", "app", { retries: 5, intervalSeconds: 5, startPeriodSeconds: 20, progressIntervalSeconds: 5, onProgress: (event) => progress.push(event) }, {
        inspectContainerByName: async () => {
            calls += 1;
            return {
                name: "demo-app",
...
test("waitForServiceReady extends retry budget and timeout to cover the startup grace period", async () => {
    let calls = 0;
    let currentTime = 0;
    await waitForServiceReadyWithDependencies("demo", "app", { retries: 20, intervalSeconds: 3, startPeriodSeconds: 300 }, {
        inspectContainerByName: async () => {
            calls += 1;
            return {
                name: "demo-app",
