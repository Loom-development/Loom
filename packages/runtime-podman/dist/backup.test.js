...
test("backupServiceToFile writes streamed backup output to disk", async () => {
    ...
    const backupPromise = backupServiceToFileWithDependencies("demo", "db", postgresService, "/tmp/backup.sql", {
...
test("backupServiceToFile rejects when the service is not running", async () => {
    await assert.rejects(() => backupServiceToFileWithDependencies("demo", "db", postgresService, resolve("/tmp", "backup.sql"), {
        isContainerRunningByName: async () => false
    }), /is not running/i);
});
test("restoreServiceFromFile copies the input file and runs the restore command", async () => {
    const commands = [];
    await restoreServiceFromFileWithDependencies("demo", "db", postgresService, "/tmp/backup.sql", {
        isContainerRunningByName: async () => true,
...
test("restoreServiceFromFile uses mysql binary mode and password env vars", async () => {
    const commands = [];
    await restoreServiceFromFileWithDependencies("demo", "db", mysqlService, "/tmp/backup.sql", {
        isContainerRunningByName: async () => true,
        ensureInputReadable: async () => undefined,
        prepareRestoreInput: async (_serviceType, inputPath) => ({ path: inputPath }),
        runPodmanCommand: async (args) => {
...
test("restoreServiceFromFile uses mariadb binary mode and password env vars", async () => {
    const commands = [];
    await restoreServiceFromFileWithDependencies("demo", "db", mariadbService, "/tmp/backup.sql", {
        isContainerRunningByName: async () => true,
        ensureInputReadable: async () => undefined,
        prepareRestoreInput: async (_serviceType, inputPath) => ({ path: inputPath }),
        runPodmanCommand: async (args) => {
...
test("restoreServiceFromFile copies sqlite backups directly into the data path", async () => {
    const commands = [];
    await restoreServiceFromFileWithDependencies("demo", "db", { type: "sqlite", image: "alpine:3.20" }, "/tmp/loom.db", {
        isContainerRunningByName: async () => true,
        ensureInputReadable: async () => undefined,
        prepareRestoreInput: async (_serviceType, inputPath) => ({ path: inputPath }),
        runPodmanCommand: async (args) => {
...
test("restoreServiceFromFile copies redis backups directly into the data path", async () => {
    const commands = [];
    await restoreServiceFromFileWithDependencies("demo", "db", { type: "redis", image: "redis:7" }, "/tmp/dump.rdb", {
        isContainerRunningByName: async () => false,
        ensureInputReadable: async () => undefined,
        prepareRestoreInput: async (_serviceType, inputPath) => ({ path: inputPath }),
        runPodmanCommand: async (args) => {
...
test("restoreServiceFromFile surfaces copy failures", async () => {
    await assert.rejects(() => restoreServiceFromFileWithDependencies("demo", "db", postgresService, "/tmp/backup.sql", {
        isContainerRunningByName: async () => true,
        ensureInputReadable: async () => undefined,
        prepareRestoreInput: async (_serviceType, inputPath) => ({ path: inputPath }),
...
test("restoreServiceFromFile rejects unsupported restore service types", async () => {
    await assert.rejects(() => restoreServiceFromFileWithDependencies("demo", "db", { type: "sqlserver", image: "mcr.microsoft.com/mssql/server:2022-latest" }, "/tmp/loom.bak", {
        isContainerRunningByName: async () => true,
        ensureInputReadable: async () => undefined,
        prepareRestoreInput: async (_serviceType, inputPath) => ({ path: inputPath }),
...
test("restoreServiceFromFile decompresses gzip-compressed mysql dumps before copying", async () => {
    ...
    try {
        await restoreServiceFromFileWithDependencies("demo", "db", mysqlService, inputPath, {
            isContainerRunningByName: async () => true,
            runPodmanCommand: async (args) => {
                commands.push(args);
                return { ok: true, stdout: "", stderr: "", code: 0 };
...
test("restoreServiceFromFile rejects xz-compressed mysql dumps with a helpful error", async () => {
    ...
    try {
        await assert.rejects(() => restoreServiceFromFileWithDependencies("demo", "db", mysqlService, inputPath, {
            isContainerRunningByName: async () => true,
            runPodmanCommand: async () => ({ ok: true, stdout: "", stderr: "", code: 0 })
        }), /xz-compressed/i);
    }
