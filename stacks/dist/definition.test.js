...
function templateServiceNames(yaml) {
...
test("pin validators reject floating versions and accept exact tag families", () => {
    ...
    for (const version of ["7.1.5", "11.1", "6.8.2-beta.1"])
        assert.doesNotThrow(() => validateGeneratorVersion(version));
});
test("definitions enforce version, aliases, canonical assets, and maintenance safety", () => {
    ...
    assert.throws(() => validateStackDefinition({ ...node, verification: [{ command: [] }] }), /verification command/i);
    assert.throws(() => validateStackDefinition({ ...node, verification: [{ command: [""] }] }), /verification command/i);
    assert.throws(() => validateStackDefinition({ ...node, verification: [{ service: "", command: ["true"] }] }), /verification service/i);
...
test("database stacks publish exact versioned package definitions and lifecycle metadata", () => {
    ...
    assert.deepEqual(stackDefinitions.filter(({ definitionVersion }) => definitionVersion === 1).map(({ id }) => id), []);
});
test("database verification checks target existing services and cover every db-all service", async () => {
    ...
    for (const id of databaseIds) {
        ...
        for (const check of checks)
            assert.ok(serviceNames.includes(check.service), `${id} service ${check.service}`);
        if (id === "db-all")
            assert.deepEqual(checks.map(({ service }) => service), serviceNames, "db-all service coverage and order");
    }
});
test("verification checks only target services declared by their templates", async () => {
    for (const definition of stackDefinitions.filter(({ definitionVersion }) => definitionVersion === 2)) {
        ...
        for (const check of definition.verification) {
            if (check.service !== undefined)
                assert.ok(serviceNames.includes(check.service), `${definition.id} service ${check.service}`);
        }
    }
});
test("Node template inventory and bytes match the approved migration fixture", async () => {
    ...
    const expected = JSON.parse(await readFile(resolve(root, "fixtures/expected.json"), "utf8"));
