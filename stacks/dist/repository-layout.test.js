...
async function repositoryFiles(...args) {
...
test("active repository content has no legacy generator-path references", async () => {
    ...
    for (const path of await repositoryFiles("--cached", "--others", "--exclude-standard")) {
        if (!isRepositorySourcePath(path))
            continue;
        if (historicalMigrationDocs.some((prefix) => path.startsWith(prefix)))
            continue;
        if (path.startsWith(runnablePrefix))
            continue;
        if (!searchableExtensions.has(extname(path)))
            continue;
...
test("generated-stack smoke refuses a pre-existing custom workspace", async () => {
    ...
    try {
        await assert.rejects(executeFile("sh", [resolve(repoDir, "scripts", "smoke-generated-stacks.sh"), "unsupported-stack"], {
            cwd: repoDir,
            encoding: "utf8",
...
//# sourceMappingURL=repository-layout.test.js.map