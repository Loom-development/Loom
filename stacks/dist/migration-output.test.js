...
const bootstrapIds = ["php-wordpress", "php-drupal", "php-symfony", "rails7", "rails7-hotwire"];
const ids = [
...
function normalizeImageDefaults(yaml, imageEnvs) {
    const declared = new Set(imageEnvs);
    return yaml.replace(/(^\s*image:\s*)\$\{([A-Z][A-Z0-9_]*):-.*?\}/gm, (match, prefix, env) => declared.has(env) ? `${prefix}\${${env}:-<IMAGE>}` : match);
}
function normalizeEnvironmentImageDefaults(envFile, imageEnvs) {
    const declared = new Set(imageEnvs);
    return envFile.replace(/^([A-Z][A-Z0-9_]*_IMAGE)=.*$/gm, (line, env) => declared.has(env) ? `${env}=<IMAGE>` : line);
}
    ...
    async function visit(current) {
        for (const entry of await readdir(current, { withFileTypes: true })) {
            ...
            if (entry.isDirectory())
                ...
                result.push(relative(directory, path).split(sep).join("/"));
        }
    }
...
test("copy stack migration preserves every non-image template byte", async () => {
    for (const id of ids) {
        ...
        const fixture = JSON.parse(await readFile(resolve(stackRoot, "fixtures/migration.json"), "utf8"));
...
test("application templates never install operating-system packages during startup", async () => {
    ...
    for (const definition of stackDefinitions) {
        if (definition.id.startsWith("db-"))
            continue;
...
test("Node-family templates run directly as the host user", async () => {
    for (const id of ["node", "astro", "node-mean", "node-mern", "node-t3"]) {
        const definition = findStackDefinition(id);
        const yaml = await readFile(resolve(root, "..", definition.assetPath, "loom.yaml"), "utf8");
...
test("Python-family templates run directly as the host user", async () => {
    for (const id of ["python", "python-django", "python-flask", "python-fastapi"]) {
        const definition = findStackDefinition(id);
        const yaml = await readFile(resolve(root, "..", definition.assetPath, "loom.yaml"), "utf8");
        assert.match(yaml, /user:\s*\$\{HOST_UID:-1000\}:\$\{HOST_GID:-1000\}/, id);
        assert.doesNotMatch(yaml, /user:\s*root|execUser:|setpriv|apt-get|apk\s+add/, id);
...
test("Rails templates run directly as the host user", async () => {
    for (const id of ["rails7", "rails7-hotwire"]) {
        const definition = findStackDefinition(id);
        const yaml = await readFile(resolve(root, "..", definition.assetPath, "loom.yaml"), "utf8");
        assert.match(yaml, /user:\s*\$\{HOST_UID:-1000\}:\$\{HOST_GID:-1000\}/, id);
        assert.doesNotMatch(yaml, /user:\s*root|execUser:|setpriv|apt-get|apk\s+add/, id);
...
test("migration normalization preserves non-image environment defaults", async () => {
    ...
    assert.notEqual(createHash("sha256").update(normalizeImageDefaults(changed, imageEnvs)).digest("hex"), createHash("sha256").update(normalizeImageDefaults(yaml, imageEnvs)).digest("hex"));
});
...
test("environment migration normalization permits declared image pins only", async () => {
    ...
    assert.equal(normalizeEnvironmentImageDefaults(env.replace(phpImage, "docker.io/library/php:8.4.99-apache"), imageEnvs), normalized);
...
test("runtime write metadata matches dependency-free Bun and Serverless templates", () => {
    for (const id of ["bun", "serverless"]) {
        const definition = findStackDefinition(id);
...
test("copy stack template image defaults exactly match their definitions", async () => {
    for (const id of ids) {
        ...
        for (const image of defaults)
            assert.equal(image.reference, imagesByEnv.get(image.env), `${id} ${image.env}`);
    }
});
...
//# sourceMappingURL=migration-output.test.js.map